...
internal static class Program
{
  ...
  private static async Task<int> Main(string[] args)
  {
    ...
    try
    {
      ...
      {
        return await NetworkConfiguratorServiceRuntime.RunAsync(args);
      }
      ...
      // Garantir apenas uma instância em execução
      using var mutex = new System.Threading.Mutex(true, @"Global\NetworkConfigurator-SingleInstance", out var isNewInstance);
      if (!isNewInstance)
      {
        MessageBox.Show(
            "O Network Configurator já está em execução.\nVerifique o ícone na área de notificação.",
            "Network Configurator",
            MessageBoxButtons.OK,
