...
internal static class IpChangeServiceRuntime
{
  public static async Task<int> RunAsync(string[] args)
  {
    ...
    builder.Services.AddWindowsService(options => options.ServiceName = IpChangeServiceProtocol.ServiceName);
    builder.Services.AddHostedService<IpChangeWorkerService>();
...
internal sealed class IpChangeWorkerService(ILogger<IpChangeWorkerService> logger) : BackgroundService
{
  private readonly ILogger<IpChangeWorkerService> _logger = logger;

  protected override async Task ExecuteAsync(CancellationToken stoppingToken)
  {
    AppLogger.Write("INFO", "Serviço ipchange iniciado.");
    ...
    while (!stoppingToken.IsCancellationRequested)
    {
      ...
      {
        _logger.LogError(ex, "Falha no serviço ipchange.");
        AppLogger.Write("ERROR", $"Falha no serviço ipchange: {ex.Message}");
  ...
  private static NamedPipeServerStream CreateServerPipe()
  {
    ...
    return NamedPipeServerStreamAcl.Create(
        IpChangeServiceProtocol.PipeName,
        PipeDirection.InOut,
        1,
